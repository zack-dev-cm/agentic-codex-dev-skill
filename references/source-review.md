...
This skill stays instruction-only, but it no longer stops at a solo patch loop. Version 0.2.2 adds explicit role definitions, `gpt-5.4`/`xhigh` policy for hard decisions, task and memory ledgers, report artifacts, source comparison, real-run acceptance checks, declared runtime requirements, and explicit invocation while preserving the clean public-surface boundary.

...
| T3 | reviewer | public surface | done | no private paths, tokens, weak public positioning, or scanner warnings | anti-bleed test, ClawHub inspect | pass: ClawHub `Security: CLEAN`, `hasWarnings: false` | GitHub CI workflow still depends on token workflow scope |
| T4 | release | GitHub and ClawHub remote surfaces | done | commit pushed and ClawHub latest is `0.3.1` with clean scanner and no warnings | `git push`, `clawhub publish`, `clawhub inspect --files` | pass: latest `0.3.1` | none |
